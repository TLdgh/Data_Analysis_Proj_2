...
MyClass <- setRefClass(
  ...
  methods = list(
    initialize = function(old_distinct,new_distinct,alpha_prior_old,alpha_prior_new,beta_prior_old,beta_prior_new,rows_per_subset){
      ...
      .self$size<-numeric(0)
      .self$posteriors<-.self$takeSubset(old_distinct,new_distinct,rows_per_subset)
    },
    ...
    takeSubset=function(old_distinct,new_distinct,rows_per_subset){
      posterior_old<-list()
      posterior_new<-list()
...
```{r}
my_instance <- MyClass$new(old_distinct,new_distinct,
                           alpha_prior_old=2,
                           alpha_prior_new=2,
                           beta_prior_old=20,
                           beta_prior_new=20)
