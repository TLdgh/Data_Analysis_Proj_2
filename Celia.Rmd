...
probability <- function(n, alpha_prior, conversion_old, beta_prior, visitor_to_old, conversion_new, visitor_to_new) {
  ...
  value <- mean(posterior_old-posterior_new)
  ...
  return(list(rate_difference = rate_difference, riemann_sum = riemann_sum, mean_old = mean_old, mean_new = mean_new))
}

results <- data.frame(n = integer(0), rate_difference = numeric(0), riemann_sum = numeric(0), mean_old = numeric(0), mean_new = numeric(0))

for (n in seq(100, 145310, by = 100)) {
  result <- probability(n, alpha_prior, conversion_old, beta_prior, visitor_to_old, conversion_new, visitor_to_new)
  result_df <- data.frame(n = n, rate_difference = result$rate_difference, riemann_sum = result$riemann_sum, mean_old = result$mean_old, mean_new = result$mean_new)
