...
sub_new <- data.frame()
result <- data.frame()
for(i in seq(100, 145200, by = 100)){
  ...
  mean_old<- mean(data.frame(old_rate=c(posterior_old))$old_rate)
  mean_new<- mean(data.frame(new_rate=c(posterior_new))$new_rate)
  value <- mean(posterior_new > posterior_old)
  sample_result <- data.frame(i = i, value = value, mean_old = mean_old, mean_new = mean_new)
...
```{r}
#Draw the scatter diagram of value
values <- result$value

scatter_data <- data.frame(i = result$i, value = values)

ggplot(scatter_data, aes(x = i, y = value)) +
  geom_point() +
  labs(x = "i", y = "Value") +
  ggtitle("Scatter Plot of i vs. Value")
```

```{r}
#Delete the column "value"
result_mean <- result[, -which(names(result) == "value")]
names(result_mean)[1] <- "n"
result_mean

result_long <- gather(result_mean, key = "Variable", value = "Value", mean_old, mean_new)

View(result_long)
```

```{r}
ggplot(result_long, aes(x = Variable, y = Value, fill = Variable)) +
  geom_boxplot() +
  labs(title = "Box Plot of mean_old and mean_new",
       x = "Variable",
       y = "Value") +
  scale_fill_manual(values = c("mean_old" = "blue", "mean_new" = "red"))
```

```{r}
ggplot(result_long, aes(x = n, y = Value, color = Variable)) +
  geom_line() +
  labs(title = "Line Plot of mean_old and mean_new",
       x = "n",
       y = "Value") +
