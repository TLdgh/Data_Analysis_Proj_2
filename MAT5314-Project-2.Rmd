...
# Introduction
The first intuitive question one can address is on the conversion rate of the visitors switching from the oldpage to the new page. in generating the correct answer to the prompt. We treat each visitor's decision as independent. The result forms a random sample of Bernoulli trials. Suppose $X_1,...,X_n$ are the random sample of the visitor's choice, with $X_i=1$ if a visitor chooses to switch to the new page with probability $p$, otherwise $X_i=0$ with probability $1-p$. The success rate $p$ is the parameter of the Bernoulli distribution. Our goal is therefore to estimate this success probability.
...
such that we would like to know if the success probability is greater than certain value $p_0$. In the frequentist approach, we often need to specify the significance level $\alpha$, which is the type I error defined by:
$$
\alpha = E[\phi(\vec{X}) | H_0] = \mathbb{P}(T(\vec{X}) \in R) = \int_R f_T(t)dt
$$
This requires us to find the distribution of the statistic $T(\vec{X})$ under the null hypothesis and its corresponding rejection region $R$, which in general is difficult. However, with Bayesian analysis one can evaluate directly the probability of the null hypothesis. We can do this by using the Monte Carlo method to evaluate the following integral:
...
\begin{align*}
f(p|\vec{x})&=\frac{f(p)f(\vec{x}|p)}{\int_0^1 f(p)f(\vec{x}|p) dp}=\frac{\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)} p^{\alpha-1+\sum_{i=1}^n x_i} (1-p)^{\beta-1+n-\sum_{i=1}^n x_i}}{\int_0^1 \frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)} p^{\alpha-1+\sum_{i=1}^n x_i} (1-p)^{\beta-1+n-\sum_{i=1}^n x_i} dp}\\
&=\frac{p^{\alpha-1+\sum_{i=1}^n x_i} (1-p)^{\beta-1+n-\sum_{i=1}^n x_i}}{\frac{\Gamma(\alpha+\sum_{i=1}^n x_i)\Gamma(\beta+n-\sum_{i=1}^n x_i)}{\Gamma(\alpha+\beta+n)}}\\
