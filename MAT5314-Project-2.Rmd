...
# Calculate the parameters for the posterior
  ...
  sigma_sim<-data.frame(sigmasq=sigma_sim) #simulate variance
  
  #pairwise multiplication
  betas_sim<-(sqrt(sigma_sim$sigmasq)*MASS::mvrnorm(n = sim_n, mu = rep(0, length(m)), CovM) + matrix(rep(c(mu), each = sim_n), ncol=length(m), byrow = FALSE))
  betas_sim<-data.frame(betas_sim)
  colnames(betas_sim)<-c(colnames(x))
  
  params<-cbind(betas_sim, sigma_sim)
  
  # Loss for betas: chosen to be the mean squared residual
  loss <- function(x, y){mean((y - x)^2)}
  bs <- apply(betas_sim, MARGIN = 2, function(x){optimize(function(y) loss(y, x), interval = c(min(x), max(x)))$minimum})

  # Loss for sigma: chosen to be the mean of the abs(residual)
  loss_sigma <- function(x, y) {mean(abs(y - x))}
  s <- apply(sigma_sim, MARGIN = 2, function(x){optimize(function(y) loss_sigma(y, x),interval = c(min(x), max(x)))$minimum}) 
...
BayesSamples<- PosteriorSample(x, y, a=8, b=100, m=as.numeric(fit$coefficients), V=diag(1,7,7) ) 
df_vline<-distinct(BayesSamples, labels, params_est)
