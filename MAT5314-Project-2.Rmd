...
$$Prior=\mathbb{P}(p)=Beta(\alpha,\beta), likelihood=\mathbb{P}(s|p)=\binom{n}{s}p^{s}(1-p)^{n-s}$$.  
By Bayesian: $\mathbb{P}(p|s)=\frac{\mathbb{P}(s|p) \cdot \mathbb{P}(p)}{\mathbb{P}(s)}$,  

\begin{align*}
\mathbb{P}(s) &= \int \mathbb{P}(s|p) \cdot \mathbb{P}(p)dp \\  
...
&=\binom{n}{s}\frac{B(\alpha+s,\beta+n-s)}{B(\alpha,\beta)} \underbrace{\int_{0}^{1}Beta(\alpha+s,\beta+n-s)dp}_{equals\ 1\ since\ integral\ on\ [0,1]\ is\ 1\ for\ Beta}\\
&=\binom{n}{s}\frac{B(\alpha+s,\beta+n-s)}{B(\alpha,\beta)} 
\end{align*}

Hence, $\mathbb{P}(p|s)=\frac{\mathbb{P}(s|p) \cdot \mathbb{P}(p)}{\mathbb{P}(s)}$  
\begin{align*}
...
cate_data <- cbind(cate_data, factor(mimic3d_clean$ExpiredHospital), factor(mimic3d_clean$LOSgroupNum))
