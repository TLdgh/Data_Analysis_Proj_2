...
for(i in model_list){
  ...
  RegressionTest<-BayesianRegression$new(train_data, responseV=c("LOSdays"), predictorV=i, prior_sigma=c(0.2,40), prior_beta=list(m=m[1:(length(i)+1)], V=diag(nrow = (length(i)+1))) )
...
```{r,eval=FALSE}
#Bayesian Regression Analysis
RegressionTest<-BayesianRegression$new(train_data, c("LOSdays"), c("age","NumProcEvents","NumChartEvents","NumDiagnosis","NumProcs","NumTransfers","TotalNumInteract","NumCallouts"), 
                                       prior_sigma=c(8,10), prior_beta=list(m=rep(0,9), V=diag(nrow = 9)) )
