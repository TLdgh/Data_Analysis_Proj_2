...
Hence, when the sample size was small, the choice of prior becomes very important; appropriate and inappropriate choices could cause a huge impact on the resulted posterior distribution. We showed the impact with an example. We extracted the first 100 rows of data and selected $Beta(2,20)$ and $Beta(1,1000)$ as the prior distributions of the conversion rate of the old page, and calculated the corresponding posterior distribution. In Figure(), the red one represented the probability density of the posterior distribution when the prior distribution was $Beta(2,20)$; the blue one represented the probability density of the posterior distribution when the prior distribution was $Beta(1,1000)$. We could see that the probability densities of these two posterior distributions was vastly different; the mean conversion rate of the old page calculated with the prior distribution of $Beta(2,20)$ was about 0.017, while the mean conversion rate of the old page calculated with the prior distribution of $Beta(1,1000)$ was about 0.145. Therefore, one must choose the parameters of the prior distribution carefully in case the sample size is small.
...
ggplot() +
  ...
  labs(title = "Posterior Density Comparison", subtitle = "Prior Beta(2,20) vs. Prior Beta(1,100)",
       x = "Conversion rate",
       y = "Density") +
...
From this point we proceeded with the Bayesian AB test on the given data. We noticed that there were some repetitive data such as columns details were the same except the "timestamp" variable and some data error like "new-page" was in the "control" group, "old-page" was in the "treatment" group. So we pre-processed the data to solve these problems and contained a clean form:
```{r}
head(new_distinct%>%select(timestamp,group,landing_page,converted),3)
```

From the previous explanations, we conclude that if the prior distribution is $Beta(\alpha,\beta)$, then the posterior is $Beta(\alpha+s,\beta+n-s)$, where $n$ is the sample size, $s$ is the summation of converted Bernoulli outcomes.
...
We first took 1000 samples to form the first subset and calculated the probability of the alternative hypothesis, i.e. $P(p_1 < p_2 |\vec{x})$, the mean of the posterior distribution for the old and new web pages (posterior_old_mean and posterior_new_mean), and finally the standard error of the posterior distributions (posterior_old_se and posterior_new_se). Then, using the existing posterior as the new prior distribution, we took another 1000 samples and add them to the first subset to form the second subset and computed the corresponding values. This was repeated until all the samples had been taken. We plotted the scatter plots of the probability of the alternative hypothesis for both the non-informative and informative priors respectively. The comparison revealed that in this data set the effect of whether the prior was informative or not was negligent, because of our big sample size.
...
frame0%>%
  ...
  labs(title = "Probability of Alternative Hypothesis, with Non-informative Prior", x = "Sample Size", y = "P")
...
frame1%>%
  ggplot(aes(x = size, y = value)) +
  geom_point(shape = 16, color = "blue") +
  labs(title = "Probability of Alternative Hypothesis, with Informative Prior", x = "Sample Size", y = "P")
```

Secondly, we plotted the convergence of the posterior mean as the sample size increases. The red line represented posterior_new_mean and the blue one represented posterior_old_mean. From the graph we noticed that when the sample size was in the range of around 200-12500, the value of posterior_new_mean was larger than the value of posterior_old_mean, i.e., new web page had higher conversion rates and the difference between the two conversion rates was quite large. However, when the sample size of the subset was larger than 12500, the blue line had been always higher than the red line, but the difference between the two was relatively small. From this, we could infer that overall, the conversion rate of the new web pages was lower than that of the old web page. This result was the opposite of what the web designer would have expected.
```{r}
...
ggplot(result_long1, aes(x = size, y = Mean, color = Variable)) +
  geom_line() +
  labs(title = "Posterior Mean Comparison with Informative Prior",
       x = "Sample Size",
       y = "Posterior Mean") +
  scale_color_manual(values = c("posterior_old_mean" = "blue", "posterior_new_mean" = "red"))
```

We plotted the 95% quantile-based credible intervals for each sample size of the subset of posterior_new_mean and posterior_old_mean. As the sample size of the subset increased, the range of the credible intervals became gradually narrower, indicating that: the larger the sample size of the data, the smaller the error caused by sampling, the closer the posterior_new_mean and posterior_old_mean were to the true value, and the results were more reliable.
...
fig_2 <- ggplot() +
  ...
  annotate("text", x = last(frame1$posterior_old_mean), y = 0, label = paste("mean of old\n", round(last(frame1$posterior_old_mean), 6)), vjust = 0.5, size = 3) +
  annotate("text", x = last(frame1$posterior_new_mean), y = 0, label = paste("mean of new\n", round(last(frame1$posterior_new_mean), 6)), vjust = 0.5, size = 3) +
...
ggplot(aes(x = x, y = value, color = density)) +
  geom_line() +
  labs(title = "Probability density function of Inverse Gamma Distribution", x = "X", y = "PDF") +
...
We proceed with an exploratory analysis of our second data set. The data set belonged to a larger database called MIMIC-III (Medical Information Mart for Intensive Care) comprising information relating to patients admitted to critical care units at a large tertiary care hospital. [@MIMIC] We first created a brief data dictionary for this data set:
...
for(i in model_list){
  ...
  candidates<-c(candidates, list(Predictors=paste(i, collapse = ", "), MeanRSS=rss))
}
candidates%>%do.call(rbind, .)%>%kable()
```

From the resulted table Table() above, we concluded that the full model has the lowest average residual sum of squares. However the second last model without the age predictor had a fairly close average RSS, but the model was one parameter less. Therefore we adopted the simpler model to make our final prediction of the test data set.
...
Finally we fitted the regression model to the data set and did a prediction of the test data.
