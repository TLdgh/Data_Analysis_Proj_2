...
The figure showed the PDF of the probability density function of the Beta distribution for five sets of values of the parameters $\alpha$ and $\beta$. From this we can see that different values of $alpha$ and $beta$ can have a great impact on the shape of the Beta distribution, the range of the distribution, and so on. 
```{r}
...
mimic3d_clean <- na.omit(subset(read.csv("mimic3d.csv", header = TRUE),select = -marital_status))
cate_data <- mimic3d_clean %>% transmute(
  ...
  LOSgroupNum=factor(LOSgroupNum)
  )

# Chi_square_test on category variables
CategoryTest <- CategoryDependence$new(cate_data, 
                                       predictor_x=c("gender","admit_type","insurance","ExpiredHospital","LOSgroupNum"),
