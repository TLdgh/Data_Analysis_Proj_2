...
data<-train_data%>%select(LOSdays, NumTransfers, NumDiagnosis, NumNotes, NumCallouts, NumProcEvents)
x<-data%>%select(-LOSdays)%>%mutate(Intercept=1, .before=NumTransfers)%>%as.matrix()
y<-matrix(data$LOSdays, ncol=1)

fit<-lm(LOSdays~NumTransfers+NumDiagnosis+NumNotes+NumCallouts+NumProcEvents, data)
print(var(fit$residuals))

BayesSamples<- PosteriorSample(x, y, a=8, b=100, m=c(0,0,0,0,0,0), V=diag(1, 6,6) ) 
...
#Prediction
test<-test_data%>%select(LOSdays,  NumTransfers, NumProcEvents, NumNotes, NumDiagnosis,NumCallouts )
...
p3<-data.frame(y=test$LOSdays, y_star=as.numeric(y_star))%>%filter(y_star>=0)%>%gather(key="data", value = "value")%>%
