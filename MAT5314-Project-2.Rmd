...
<<<First, we needed to preprocess the data. We found that 不是所有不同的user_id都只对应唯一一种landing status。“。。。代码”<<<
```{r}
# Part 1
...
We first took 100 samples to form the first subset and calculated the value (the proportion of posterior samples from the new page that were greater than the posterior samples from the old page), posterior_old_mean and posterior_new_mean (the mean of the posterior distribution for the old and new web pages), posterior_old_se and posterior_new_se (standard error of the posterior distributions). 
